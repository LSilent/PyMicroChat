...
TEST_KEY_WORD = ('测试分享链接', '测试好友列表','图灵机器人','自动通过好友申请')
# 测试开关
TEST_STATE    = [1,1,1,1]
...
def tuling_robot(msg):
    ...
    try:
        robot_ret = eval(Util.post(TULING_HOST, TULING_API,
                                   json.dumps(data)).decode())
        logger.debug('tuling api 返回:{}'.format(robot_ret))
        # 自动回消息
        interface.new_send_msg(
            msg.from_id.id, robot_ret['results'][0]['values']['text'].encode(encoding="utf-8"))
    except:
...
def test(msg):
    ...
    if '测试' == msg.raw.content:  # help
        interface.new_send_msg(
            msg.from_id.id, '当前支持的测试指令:'.encode(encoding="utf-8"))
        for i in range(len(TEST_KEY_WORD)):
            interface.new_send_msg(msg.from_id.id, '[{}]{}({})'.format(
                i, TEST_KEY_WORD[i],state(TEST_STATE[i])).encode(encoding="utf-8"))
        return False
    elif TEST_KEY_WORD[0] == msg.raw.content or '0' == msg.raw.content:                                                             # 测试分享链接
        interface.send_app_msg(msg.from_id.id, '贪玩蓝月', '大渣好,我系咕天乐,我是渣渣辉,贪挽懒月,介系一个你没有挽过的船新版本', 'http://www.gov.cn/',
                               'https://ss0.bdstatic.com/-0U0bnSm1A5BphGlnYG/tam-ogel/f1d67c57e00fea1dc0f90210d7add1ad_121_121.jpg')
        return False
